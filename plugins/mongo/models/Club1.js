...
export default class Club1 extends ModelHelper {
  constructor(club1Data) {
    super('club1s');
    this.modelFields = {
    ...
    if (club1Data) {
      for (let key in this.modelFields) {
        if (club1Data[key] !== undefined) {
          this.modelFields[key].value = club1Data[key];
        }
      }
    }
  }
  ...
  static getModelFields() {
    return Object.keys(new Club1().modelFields).map((key) => {
      const field = new Club1().modelFields[key];
    ...
    try {
      if (req.files) {
        for (const key in req.files) {
          ...
          const fileKey = `club1s/${Date.now()}-${file.originalname}`;
