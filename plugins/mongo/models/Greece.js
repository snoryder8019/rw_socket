...
export default class Greece extends ModelHelper {
  constructor(greeceData) {
    super('greeces');
    this.modelFields = {
    ...
    if (greeceData) {
      for (let key in this.modelFields) {
        if (greeceData[key] !== undefined) {
          this.modelFields[key].value = greeceData[key];
        }
      }
    }
  }
  ...
  static getModelFields() {
    return Object.keys(new Greece().modelFields).map((key) => {
      const field = new Greece().modelFields[key];
    ...
    try {
      if (req.files) {
        for (const key in req.files) {
          ...
          const fileKey = `greeces/${Date.now()}-${file.originalname}`;
