...
export default class Alaska extends ModelHelper {
  constructor(alaskaData) {
    super('alaskas');
    this.modelFields = {
    ...
    if (alaskaData) {
      for (let key in this.modelFields) {
        if (alaskaData[key] !== undefined) {
          this.modelFields[key].value = alaskaData[key];
        }
      }
    }
  }
  ...
  static getModelFields() {
    return Object.keys(new Alaska().modelFields).map((key) => {
      const field = new Alaska().modelFields[key];
    ...
    try {
      if (req.files) {
        for (const key in req.files) {
          ...
          const fileKey = `alaskas/${Date.now()}-${file.originalname}`;
