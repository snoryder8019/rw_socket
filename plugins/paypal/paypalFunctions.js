...
export const createOrder = async (req, res) => {
  ...
  try {
    ...
    if (approvalLink) {
      ...
      res
        .status(500)
        .json({ error: 'Payer-action link not found in PayPal response.' });
    }
  } catch (error) {
...
export const returnPaypalSuccess = async (req, res) => {
...
export const getCheckoutAwaiting = async (req, res, card) => {
  ...
  console.log(
    `Received query parameters - UserID: ${userId}, CardID: ${cardId}, OrderID: ${customId}`
  );
