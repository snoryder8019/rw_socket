...
import { updatePaypalOrder } from '../dbFunctions.js';

export const checkouts = async (req, res) => {
    ...
    if (eventType == 'CHECKOUT.ORDER.COMPLETED') {
      ...
      if (purchaseUnits) {
        purchaseUnits.forEach((unit, index) => {
          ...
          console.log(
            `Amount: ${unit.amount.value} ${unit.amount.currency_code}`
          );
      ...
      const payerEmail = req.body.resource.payer.email_address;
      console.log(
        `Event received: OrderId: ${orderId}, EventType: ${eventType}, Status: ${status}`
      );
        ...
        purchaseUnits.forEach((unit, index) => {
          ...
          if (unit.items) {
            unit.items.forEach((item, itemIndex) => {
              ...
              console.log(
                `    Unit Price: ${item.unit_amount.value} ${item.unit_amount.currency_code}`
              );
            });
          }
      ...
      console.log(
        `Other hooks: ${eventType}, Summary: ${summary}, Order ID: ${orderId}`
      );
    }
