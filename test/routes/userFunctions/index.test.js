...
const loadRoutes = async () => {
  const baseDir = path.join(__dirname, '../../../routes/userFunctions');
  try {
...
(async () => {
  ...
  describe('Dynamic Route Tests for gamesFunctions', function () {
    this.timeout(30000); // Increase timeout to 30 seconds to prevent timeout issues

    let testDirectories;
    before(async function () {
      try {
        testDirectories = await fs.readdir(path.join(__dirname, '../../../routes/userFunctions'));
    ...
    testDirectories.forEach((dir) => {
      routesToTest.forEach((route) => {
        describe(`Testing ${route} route in ${dir} module`, () => {
          ...
          it(`should return 200 OK for valid requests with user user`, (done) => {
            console.log(chalk.blue(`Testing ${route} with user user in module: ${dir}`));
            ...
            request(app)
              ...
              .end((err, res) => {
                if (err) {
                  console.error(chalk.red(`Error testing ${route} with user user in module: ${dir}`, err));
                  return done(err);
                }
                ...
                console.log(chalk.green(`🎉 Successfully tested ${route} with user user in module: ${dir} 🎉`));
                done();
              });
          });
        });
